...
async function doAnalyze(){
    ...
    let price,pChg,vol,qV,s50,s200,e20,e50,wma,adxV,tsiV,wllV,psrV,voV;
    let thDir=null,stFG=null,stSig=null,thSigD=null,staInd=null;

    if(useBE&&api){
        price=xV(api.techPrice);vol=xV(api.volume);s50=xV(api.sma50);s200=xV(api.sma200);
        e20=xV(api.ema20);e50=xV(api.ema50);wma=xV(api.wma14);adxV=xV(api.adx);
        tsiV=xV(api.tsi);wllV=xV(api.williams);psrV=xV(api.psar);voV=xV(api.volOsc);
        thDir=xTH(api.thSignal);if(api.thSignal?.success)thSigD=api.thSignal.data;
        if(api.fearGreed?.success)stFG=api.fearGreed.data;if(api.stSignal?.success)stSig=api.stSignal.data;
        if(api.staIndicators?.success)staInd=api.staIndicators.data;
        const sts=[api.techPrice?.success,api.sma50?.success||api.ema20?.success,api.adx?.success||api.tsi?.success,api.tvQuote?.success,api.aiSignal?.success,api.news?.success,api.thSignal?.success,api.fearGreed?.success||api.stSignal?.success,api.staIndicators?.success];
        const lts=[api.techPrice?.latency||0,Math.max(api.sma50?.latency||0,api.ema20?.latency||0),Math.max(api.adx?.latency||0,api.tsi?.latency||0),api.tvQuote?.latency||0,api.aiSignal?.latency||0,api.news?.latency||0,api.thSignal?.latency||0,Math.max(api.fearGreed?.latency||0,api.stSignal?.latency||0),api.staIndicators?.latency||0];
        apiSet(sts,lts);
    }
    if(bD){
