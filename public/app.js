...
async function doAnalyze(){
    ...
    let thDir=null,stFG=null,stSig=null,thSigD=null,staInd=null,futSig=null,quantSig=null,dnAnalysis=null,qsSig=null;
    ...
    if(useBE&&api){
        ...
        // H7: Futures signal
        if(api.futSignal?.success&&api.futSignal?.data)futSig=api.futSignal.data;
        // H9: QuantSignalPro
        if(api.quantSignals?.success&&api.quantSignals?.data)quantSig=api.quantSignals.data;
        // H10: DataNest
        if(api.datanestAnalysis?.success&&api.datanestAnalysis?.data)dnAnalysis=api.datanestAnalysis.data;
        // H11: Quicksilver
        if(api.qsSignals?.success&&api.qsSignals?.data)qsSig=api.qsSignals.data;
    ...
    // H7: Futures signal influence
    if(futSig){const fs=futSig.signal||futSig.action||futSig.recommendation||'';if(String(fs).toUpperCase().match(/BUY|LONG|BULL/))bu++;else if(String(fs).toUpperCase().match(/SELL|SHORT|BEAR/))be++}
    // H9: QuantSignalPro influence
    if(quantSig){
        const qs=Array.isArray(quantSig)?quantSig:typeof quantSig==='object'?[quantSig]:[];
        qs.forEach(q=>{const s=q.signal||q.action||q.direction||q.recommendation||'';
        if(String(s).toUpperCase().match(/BUY|LONG|BULL/))bu++;
        else if(String(s).toUpperCase().match(/SELL|SHORT|BEAR/))be++});
    }
    // H10: DataNest analysis influence
    if(dnAnalysis){
        const da=dnAnalysis.recommendation||dnAnalysis.signal||dnAnalysis.rating||dnAnalysis.analysis||'';
        if(String(da).toUpperCase().match(/BUY|LONG|BULL|OUTPERFORM|OVERWEIGHT/))bu++;
        else if(String(da).toUpperCase().match(/SELL|SHORT|BEAR|UNDERPERFORM|UNDERWEIGHT/))be++;
    }
    // H11: Quicksilver signals influence
    if(qsSig){
        const qArr=Array.isArray(qsSig)?qsSig:typeof qsSig==='object'?[qsSig]:[];
        qArr.forEach(q=>{const s=q.signal||q.action||q.direction||q.side||'';
        if(String(s).toUpperCase().match(/BUY|LONG|BULL/))bu++;
        else if(String(s).toUpperCase().match(/SELL|SHORT|BEAR/))be++});
    }
    // H8: Forex/Gold Trend influence
    if(useBE&&api?.fxTrend?.success&&api.fxTrend.data){
        const fxD=api.fxTrend.data;
        // Check if XAUUSD or pair trend matches
        const entries=Array.isArray(fxD)?fxD:typeof fxD==='object'?Object.values(fxD):[];
        entries.forEach(e=>{
            const trend=e?.trend||e?.direction||e?.signal||'';
            if(String(trend).toUpperCase().match(/UP|BUY|BULL|STRONG/))bu++;
            else if(String(trend).toUpperCase().match(/DOWN|SELL|BEAR|WEAK/))be++;
        });
    }
