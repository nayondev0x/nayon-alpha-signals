...
// Render gives its own PORT — must use process.env.PORT
...
async function fetchAPI(host,ep,params={}){
    const qs=new URLSearchParams(params).toString();
    const url=`https://${host}${ep}${qs?'?'+qs:''}`;
    const s=Date.now();
    try{
        const r=await fetch(url,{method:'GET',headers:{'x-rapidapi-key':KEY,'x-rapidapi-host':host}});
...
const h1=(ep,p)=>fetchAPI(H1,ep,p);
const h2=(ep,p)=>fetchAPI(H2,ep,p);
const h3=(ep,p)=>fetchAPI(H3,ep,p);
const h4=(ep,p)=>fetchAPI(H4,ep,p);
const h5=(ep,p)=>fetchAPI(H5,ep,p);
...
// ===== HOST 5: Stock Technical Analysis =====
...
app.listen(PORT, '0.0.0.0', ()=>{
    ...
    console.log(`Nayon Alpha Signals v5.0 — NayonDev`);
